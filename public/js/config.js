 ...
 *
 * «Артём проработал N часов» за день считается из цифр его выгрузки
 * (обслуженные компании, счета, услуги/отчёты) по нормативу времени.
 *
 * TODO(harry-excel): сейчас норматив — это оценочное сопоставление ниже
 *   (DAILY_REPORT.serviceMinutes). Позже хронометраж придёт из Excel Гарри
 *   (реальное время по каждой услуге). Тогда достаточно переписать ОДНУ функцию
 *   `serviceHours()` — заменить расчёт по конфигу на чтение реальной таблицы,
 *   остальной код отчёта трогать не нужно.
 *
 * Цифры дня (figures): { companies, invoices, services }.
 *   companies — сколько компаний бухгалтера с активностью в выгрузке за день;
 *   invoices  — счета (выставленные + полученные, ArmSoft + TaxService);
 *   services  — сданные отчёты / налоговые услуги (TaxService).
 * -------------------------------------------------------------------------- */
const DAILY_REPORT = {
  // Норматив времени на единицу работы (минуты). Заглушка до хронометража Гарри.
  serviceMinutes: {
    company:  15,   // минут на обслуживание одной компании за день
    invoice:   5,   // минут на один счёт (выставленный/полученный)
    service:  30,   // минут на одну услугу/сданный отчёт
  },
  // Русские подписи цифр (используются и в пометках, и в письме Артёму)
  figureLabels: {
    companies: 'Компании (с активностью)',
    invoices:  'Счета',
    services:  'Услуги / отчёты',
  },
};

/**
 * ЕДИНАЯ точка расчёта «сколько часов проработал Артём» за день по его цифрам.
 * TODO(harry-excel): заменить тело на чтение реального хронометража (таблица
 * времени по услугам из Excel Гарри). Сигнатуру и место вызова менять не нужно.
 * @param {{companies:number, invoices:number, services:number}} figures
 * @returns {number} часы (с округлением до 0.1)
 */
function serviceHours(figures) {
  const m = DAILY_REPORT.serviceMinutes;
  const f = figures || {};
  const minutes =
      Number(f.companies || 0) * m.company
    + Number(f.invoices  || 0) * m.invoice
    + Number(f.services  || 0) * m.service;
  return Math.round((minutes / 60) * 10) / 10;
}

// экспорт для среды Node (тесты); в браузере функция и так глобальна
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CONFIG, RULES, DAILY_REPORT, serviceHours };
}
